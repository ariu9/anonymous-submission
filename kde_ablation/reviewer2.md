...
By contrast, detecting interaction units solely from pixel-level behavior remains challenging in fast-paced gameplay with frequent motion, camera shifts, overlapping effects, and simultaneous multi-agent actions. We therefore use visible HUD cues as reliable anchors for accurate interaction timing in the current work. At the same time, we view direct visual behavior modeling for low-UI gameplay as an important future direction.
...
We include a video demo of the full pipeline, including highlight extraction from full gameplay videos and the corresponding generated captions.
